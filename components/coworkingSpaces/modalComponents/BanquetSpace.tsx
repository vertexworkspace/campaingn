  ...
  return (
    <section className="lg:pl-8 lg:pe-3 mt-10 lg:mt-20 pb-10 lg:pb-8 bg-white text-gray-800">
      ...
      <div className="text-center mb-8 lg:mb-20 px-4 sm:px-0 sm:mb-20">
        <h2 className="text-2xl md:text-[40px] text-primary font-semibold mb-6 sm:mb-8">
        ...
        <div className="lg:grid grid-cols-1 hidden  sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 px-4 sm:px-0">
          {features.map((f, i) => (
            <div
              key={i}
              className="p-5 sm:p-6 border border-[#0097DC]  flex flex-col items-center text-center"
            >
              <div className="mb-3 sm:mb-4">{f.icon}</div>
              <p className="text-sm sm:text-base md:text-lg text-secondary font-medium">
                {f.title}
              </p>
            </div>
          ))}
         ...
         spaceBetween={24}
          ...
          {features.map((f, i) => (
            ...
            <div
              key={i}
              className="p-5  sm:p-6 border border-[#0097DC] lg:hidden flex flex-col items-center text-center"
            >
              <div className="mb-3 sm:mb-4">{f.icon}</div>
              <p className="text-sm sm:text-base md:text-lg text-secondary font-medium">
                {f.title}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center px-4 sm:px-0">
        ...
        <div className="lg:col-span-7">
          <h3 className="text-2xl md:text-[40px] text-primary font-semibold mb-4 sm:mb-6">
            Banquet Space
          </h3>
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-2 mb-6">
            {details.map((item, index) => (
              <li
                key={index}
                className="flex items-start gap-3  p-3 sm:p-0  sm:bg-transparent"
              >
 ...
 <div className="w-full lg:hidden mb-11 lg:col-span-5">
          ...
          <div className="space-y-4 mb-6">
            <div className="border border-[#0097DC]  py-4 px-5 sm:px-7 flex flex-row justify-between items-start sm:items-center gap-3 sm:gap-0 transition">
              <div>
                <p className="text-base sm:text-lg lg:text-[28px]">
                  Weekday Corporate Events
                </p>
                <p className="text-sm sm:text-lg text-secondary">(4-hour slot)</p>
              </div>
              <div className="text-left sm:text-right">
                <p className="text-[22px] sm:text-[29px] font-extrabold text-[#0097DC]">
                  ₹1,000
                </p>
                <p className="text-[#0097DC] text-sm sm:text-lg font-extrabold">
                  + GST
                </p>
              </div>
            </div>
...
<div className="flex lg:block justify-center">
         <Button onClick={()=>setOpen(true)} aria-label="Get a Quote" className="bg-[#0097DC] hover:bg-[#0097DC]/80 text-white font-semibold px-6 py-2">
            Book Now
          </Button>
        </div>
        </div>
